...
window.addEventListener("message", ({ data: { subjectNames } = {} }) => {
  if (subjectNames) {
    ...
    dropdown.innerHTML = `<option value=""></option>${subjectNames
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function adjustDropdownHeight() {
    const optionHeight =
      subjectDropdown.querySelector("option:checked")?.offsetHeight;
...
subjectDropdown.addEventListener("change", function () {
  ...
  const headingElement2 = document.getElementById(
    "add-question-form-heading-ab"
  );
  ...
  if (
    document.getElementById("add-question-form-ab").style.display === "block" &&
    selectedSubject &&
...
clearButton.addEventListener("click", () => {
  ...
  currentTypingElements.forEach(({ typingInterval }) =>
    ...
    fetchData(currentDataFile)
      .then((data) =>
        processAndDisplayResults(
          data,
          searchTerm,
          currentDataFile === "ab.json",
          currentSection
        )
      )
      .catch((error) =>
        console.error(`Error fetching ${currentDataFile}:`, error)
      );
  }, 100);
}
  ...
  function typeNextQuestion(index) {
    ...
    if (isAbJson) {
      ...
      content = `
                ...
                <h5>${convertNewlinesToHtml(
                  processTextWithImages(questionData.question)
                )}</h5>
                <h6>Explanation: ${convertNewlinesToHtml(
                  processTextWithImages(questionData.explanation)
                )}</h6>
      ...
      content = `
                <h4>${section}</h4>
                <h5>${convertNewlinesToHtml(
                  processTextWithImages(question.question)
                )}</h5>
                <ul>${question.options
                  .map(
                    (option) =>
                      `<li>${convertNewlinesToHtml(
                        processTextWithImages(option)
                      )}</li>`
                  )
                  .join("")}</ul>
                <h6>Answer: ${convertNewlinesToHtml(
                  processTextWithImages(question.correctAnswer)
                )}</h6>
...
async function fetchData(file) {
  for (const baseUrl of urls) {
    try {
      ...
      if (!response.ok) {
        console.warn(
          `Failed to fetch from ${baseUrl}${file}. Status: ${response.status}`
        );
...
function processAndDisplayResults(
  ...
  const normalizedSearchTerm = searchTerm
    .trim()
    .toLowerCase()
    .replace(/\s+/g, " "); // Normalize search term
  ...
  if (isAbJson) {
    ...
    matchingQuestions = sectionData
      .map((q, index) => {
        const normalizedQuestion = q.question
          .trim()
          .toLowerCase()
          .replace(/\s+/g, " "); // Normalize question text
        return {
          section,
          questionData: q,
    ...
    const sectionData =
      data.sections?.find(
        (s) => s.section?.toLowerCase() === currentSubject.toLowerCase()
      )?.questions || [];
    matchingQuestions = sectionData
      .map((q, index) => {
        const normalizedQuestion = q.question
...
async function deleteQuestion(button, isAbJson = false) {
  ...
  const subject =
    questionElement.querySelector("h4")?.textContent?.trim() || null;
    ...
    for (const baseUrl of urls) {
      try {
        ...
        if (response.ok) {
          ...
          console.warn(
            `Failed to delete from ${baseUrl}${endpoint}. Status: ${response.status}`
          );
        }
      } catch (error) {
...
function addImageZoomListeners(container) {
  container.querySelectorAll(".image-container").forEach((imgContainer) => {
    ...
    function updatePosition() {
      ...
      imgContainer.scrollLeft = Math.max(
        0,
        Math.min(imgContainer.scrollLeft, imgRect.width - rect.width)
      );
      imgContainer.scrollTop = Math.max(
        0,
        Math.min(imgContainer.scrollTop, imgRect.height - rect.height)
      );
    }
    ...
    function handleZoom(e, touch = false) {
      ...
      currentZoom =
        zoomLevels[(zoomIndex = (zoomIndex + 1) % zoomLevels.length)];
      const rect = imgContainer.getBoundingClientRect();
      const x = touch
...
async function editQuestion(button, isAbJson = false) {
  ...
  questionElement
    .querySelectorAll("h4, h5, h6, ul, button")
  ...
  if (isAbJson) {
    ...
    let processedQuestionText = processTextWithImages(
      questionElement.querySelector("h5").innerHTML,
      true
    );
    let processedExplanationText = processTextWithImages(
      questionElement.querySelector("h6").innerHTML,
      true
    ).replace(/^Explanation:\s*/, "");

    editInput.value = normalizeAndExtractText(processedQuestionText);
    editExplanationInput.value = normalizeAndExtractText(
      processedExplanationText
    );
    ...
    saveButton.addEventListener("click", async () => {
      ...
      let newExplanation = normalizeAndExtractText(
        editExplanationInput.value.trim()
      );
    ...
    const questionText = normalizeAndExtractText(
      processTextWithImages(questionElement.querySelector("h5").innerHTML, true)
    );
    const options = Array.from(questionElement.querySelectorAll("li")).map(
      (li) => normalizeAndExtractText(processTextWithImages(li.innerHTML, true))
    );
    const answer = normalizeAndExtractText(
      processTextWithImages(questionElement.querySelector("h6").innerHTML, true)
    ).replace(/^Answer:\s*/, "");
    ...
    saveButton.addEventListener("click", async () => {
      ...
      let newOptions = Array.from(
        editOptionsList.querySelectorAll("textarea")
      ).map((input) => normalizeAndExtractText(input.value.trim()));
      let newAnswer = normalizeAndExtractText(editAnswerInput.value.trim());
      if (
        newQuestion.length === 0 ||
        newOptions.some((option) => option.length === 0) ||
        newAnswer.length === 0
      ) {
  ...
  cancelButton.addEventListener("click", () => {
    ...
    document
      ...
      .forEach((result) => (result.style.display = "block"));
    questionElement
      .querySelectorAll("h4, h5, h6, ul, button")
...
function resetUploadedImages() {
  ...
  const messageDivSelectors = [
    "#image-placeholder-message",
    "#image-placeholder-message-ab",
  ];
  ...
  document
    .querySelectorAll(".highlighted-placeholder")
  ...
  console.log(
    "Uploaded images, formData, and imageReferences have been reset."
  );
}
  ...
  for (const baseUrl of urls) {
    try {
      ...
      if (response.ok) {
        ...
        console.warn(
          `Failed to send data to ${baseUrl}${currentDataFile}. Status: ${response.status}`
        );
      }
    } catch (error) {
      console.error(
        `Error sending data to ${baseUrl}${currentDataFile}:`,
        error
      );
    }
  }
...
function switchToAddFormMcq() {
  ...
  if (
    formAB.style.display === "block" &&
    currentDataFile === "questions.json"
  ) {
    formAB.style.display = "none";
    resetAddForm();
    formMCQ.style.display = "block";
...
document.addEventListener("click", function () {
  if (
    document.getElementById("add-question-form-mcq").style.display ===
      "block" &&
    this.value !== "" &&
    this.value !== "Select Subject"
  ) {
    switchToAddFormAB();
    isModifyActive = false;
  }
...
document
  .getElementById("cancel-add-question-mcq")
...
document
  .getElementById("cancel-add-question-ab")
  .addEventListener("click", function () {
...
function handleImageUploads(
  ...
  if (!fileInput || !messageDiv) {
    console.error(
      `Element not found for selector: ${fileInputSelector} or ${messageSelector}`
    );
    return () => [];
  }
      ...
      if (file.type.startsWith("image/")) {
        ...
        reader.onload = function (e) {
          ...
          const currentPlaceholders = Array.from(imageMap.keys())
            .map(
              (ph) =>
                `<button class="highlighted-placeholder" data-placeholder="${ph}" aria-label="Show image ${ph}" disabled>${ph}</button>`
            )
            .join(", ");
          ...
          if (!typingComplete) {
            ...
            if (
              existingMessage.includes(
                "<strong>where you want to add the image</strong>"
              )
            ) {
              updatedMessage = existingMessage.replace(
                /<strong>where you want to add the image<\/strong>/,
                ` ${newPlaceholderText} <br> <strong>where you want to add the image</strong>`
              );
            } else {
              updatedMessage = existingMessage.replace(
                /<br>$/,
                ` ${newPlaceholderText} <br>`
              );
            }
            startTyping();
  ...
  function handleButtonClick(event) {
    ...
    console.log(
      "Image source:",
      imageSrc ? imageSrc.substring(0, 50) + "..." : "not found"
    );
  ...
  function setupZoomListeners(container, img) {
    ...
    function updatePosition() {
      ...
      container.scrollLeft = Math.max(
        0,
        Math.min(container.scrollLeft, imgRect.width - rect.width)
      );
      container.scrollTop = Math.max(
        0,
        Math.min(container.scrollTop, imgRect.height - rect.height)
      );
    }
...
const getMCQImageFiles = handleImageUploads(
  "#upload-question-images",
  "#image-placeholder-message"
);
const getABImageFiles = handleImageUploads(
  "#upload-ab-images",
  "#image-placeholder-message-ab"
);
...
function validateFormInputs(
  ...
  function isFieldValid(field) {
    return (
      field.trim() !== "" ||
      /\(.*\.(?:gif|jpe?g|tiff?|png|webp|bmp|svg|ico|avif)\)/.test(field)
    );
  }

  if (isMCQ) {
    return (
      isFieldValid(questionText) &&
      optionsText.every(isFieldValid) &&
      isFieldValid(answerText)
    );
  } else {
...
function saveQuestion({
  ...
  if (
    ...
    return displayAlert(
      "Please fill all required fields with either text or image placeholders."
    );
  }
  ...
  function formatImageFileNames(text) {
    return typeof text === "string"
      ? text.replace(
          /\b([A-Za-z0-9_-]+\.(gif|jpe?g|tiff?|png|webp|bmp|svg|ico|avif))\b/g,
          "image/$1"
        )
      : text;
  }
  ...
  imageFiles.forEach((file) =>
    imageMap.set(file.name.replace(/\s+/g, "_"), file)
  );

  const formattedQuestionText = formatImageFileNames(questionText);
  const formattedOptionsText = optionsText.map((opt) =>
    formatImageFileNames(opt)
  );
  ...
  async function fetchDataFromUrls(urls) {
    for (const baseUrl of urls) {
      try {
        ...
        if (!response.ok) {
          console.warn(
            `Failed to fetch from ${baseUrl}${endpoint}. Status: ${response.status}`
          );
          continue;
        }
        return response.json();
  ...
  fetchDataFromUrls(urls)
    .then((data) => {
      ...
      if (!isMCQ) {
        ...
        const sectionData = data.sections.find(
          (section) => section.section === currentSubject
        );
        if (sectionData) {
          ...
          for (const baseUrl of urls) {
            try {
              ...
              document.getElementById(
                isMCQ ? "add-question-form-mcq" : "add-question-form-ab"
              ).style.display = "none";
              return;
            } catch (error) {
    ...
    .catch((error) => {
      ...
      displayAlert(
        `Failed to check existing questions. Error: ${error.message}`
      );
    });
}
...
document
  .getElementById("save-new-question-ab")
  .addEventListener("click", () => {
